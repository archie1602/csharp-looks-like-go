...
The `bin/main.dSYM/` folder next to the binary is a macOS debug-symbol bundle auto-generated by the AOT toolchain; safe to delete for distribution. EF Core emits a few trim/AOT analyzer warnings (`IL2026`/`IL3050`) because parts of it still use reflection, but the AOT-compiled binary works end-to-end.
